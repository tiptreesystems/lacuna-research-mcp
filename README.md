...
`get_author_context` with `view="full"` slices large `papers`, `impact_clusters`, and `levels.cluster` arrays by default with a limit of 50 and a maximum requested limit of 200. Responses include `*_total`, `*_returned`, `*_truncated`, and `truncation` metadata. Pass offsets to page through those arrays, or set `full=true` to skip MCP-side slicing. The upstream server collections remain capped at 100 even with `full=true`. (The default compact view is capped server-side, so these MCP-side slicing params and verbose truncation metadata apply only to `view="full"`.)
